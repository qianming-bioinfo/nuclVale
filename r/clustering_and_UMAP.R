...
option_list <- list(
  ...
  # make_option(c("-k", "--desired_number_of_clusters"), type = "numeric", 
...
# plot_1
plot_1 <- plot_ly(
  x = factor(names(sorted_cv), levels = names(sorted_cv)), 
  y = sorted_cv, 
  type = 'bar'
) %>%
  layout(
    title = "Coefficient of Variation of Motifs",
    xaxis = list(title = "Motifs"),
    yaxis = list(title = "CV")
  )

saveWidget(plot_1, paste0(fig_out_dir, "/CV_plot.html"), selfcontained = F)
...
## plot_2
plot_2 <- ggplot(UMAP_cluster, aes(x = umap1, y = umap2, color = as.factor(cluster))) +
  geom_point(size = 2.5) +
  labs(x = "UMAP_1", y = "UMAP_2") +
  theme_classic() +
  labs(color = "clusters")

ggsave(paste0(fig_out_dir, "/umap_plot.png"), plot = plot_2, dpi = 1200)
ggsave(paste0(fig_out_dir, "/umap_plot.pdf"), plot = plot_2)
